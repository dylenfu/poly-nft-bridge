...
import (
	...
	"github.com/polynetwork/poly-nft-bridge/dao/crosschaindao/explorerdao"
...
func (eff *ExplorerEffect) updateHash() error {
	...
	eff.db.Table("mchain_tx").
		Where("left(mchain_tx.ftxhash, 8) = ? and mchain_tx.fchain != ?", "00000000", basedef.ETHEREUM_CROSSCHAIN_ID).
		Select("mchain_tx.txhash as poly_hash, fchain_tx.txhash as src_hash").
		Joins("inner join fchain_tx on mchain_tx.ftxhash = fchain_tx.xkey and mchain_tx.fchain = fchain_tx.chain_id").
		Preload("SrcTransaction").
		Preload("PolyTransaction").
		Find(&polySrcRelations)
