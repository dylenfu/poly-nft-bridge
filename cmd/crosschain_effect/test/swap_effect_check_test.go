...
func TestSwapEffect_Run(t *testing.T) {
	...
	fmt.Printf("current directory: %s\n", dir)
	config := conf.NewConfig("./../../conf/config_testnet.json")
	server := basedef.SERVER_POLY_SWAP

	if config == nil {
		panic("read config failed!")
	}
	effect := crosschain_effect.NewEffect(server, config.EventEffectConfig, config.DBConfig)
	if effect == nil {
		panic("monitor is not valid")
	}
	crossChainEffect := crosschain_effect.NewCrossChainEffect(effect)
