...
func TestSwapEffect_AddSrcTransaction(t *testing.T) {
	...
	dao := crosschaindao.NewCrossChainDao(basedef.SERVER_POLY_SWAP, backup, config.DBConfig)
	if dao == nil {
...
func TestSwapEffect_AddDstTransaction(t *testing.T) {
	...
	dao := crosschaindao.NewCrossChainDao(basedef.SERVER_EXPLORER, backup, config.DBConfig)
	if dao == nil {
		panic("server is not valid")
	}
