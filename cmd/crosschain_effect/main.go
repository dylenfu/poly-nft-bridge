...
import (
	...
	"github.com/polynetwork/poly-nft-bridge/logic/effect"
...
func startServer(ctx *cli.Context) {
	...
	effect.StartCrossChainEffect(config.Server, config.EventEffectConfig, config.DBConfig)
}
...
func waitSignal() os.Signal {
	...
	go func() {
		for sig := range sc {
			logs.Info("cross chain effect received signal:(%s).", sig.String())
			exit <- sig
			close(exit)
			break
		}
