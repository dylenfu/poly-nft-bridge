...
func startServer(ctx *cli.Context) {
	logs.SetLogger(logs.AdapterFile, `{"filename":"logs/fee_listen.log"}`)
	...
	fee.StartFeeListen(config.Server, config.FeeUpdateSlot, config.FeeListenConfig, config.DBConfig)
}
...
func waitSignal() os.Signal {
	...
	go func() {
		for sig := range sc {
			logs.Info("chain fee listen received signal:(%s).", sig.String())
			exit <- sig
			close(exit)
			break
		}
